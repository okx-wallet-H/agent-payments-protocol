...
assertIncludes(webHumanPreview, 'const [email, setEmail] = useState("")', "root web preview does not prefill a demo email");
...
assertIncludes(webHumanPreview, "return <HumanAgentChatHome email={normalizedEmail}", "root web preview passes the verified email into the human Agent shell");
assertIncludes(webHumanPreview, "function HumanAgentChatHome", "root web preview has a dedicated human Agent post-login shell");
assertIncludes(webHumanPreview, "function HumanAgentChatHome({ email }: { email?: string })", "root web preview Agent shell receives the verified email");
assertIncludes(webHumanPreview, '<span>{email || "邮箱已验证"}</span>', "root web preview settings show the verified email instead of fixture data");
assertNotIncludes(webHumanPreview, "demo@hwallet.vip", "root web preview does not expose the old demo email fixture");
...
assertNotIncludes(webHumanPreview, "function buildAgentReply", "root web preview no longer builds fake local Agent replies");
assertIncludes(webHumanPreview, '"/api/v2/phase-one"', "root web preview sends Agent messages through the real phase-one route");
assertIncludes(webHumanPreview, "createHumanPreviewUserId(email)", "root web preview passes a stable preview user id into the real route");
assertIncludes(webHumanPreview, "createHumanAgentMessageFromTurn", "root web preview renders the real mobile turn result");
assertIncludes(webHumanPreview, "const [agentBusy, setAgentBusy] = useState(false)", "root web preview guards against duplicate sends while the real route is running");
assertIncludes(webHumanPreview, "setMessages([nextMessage, pendingMessage])", "root web preview clears old messages before the real route returns");
assertIncludes(webHumanPreview, "setMessages([nextMessage, assistantMessage])", "root web preview replaces pending state with the real Agent result");
assertIncludes(webHumanPreview, "setMessages([nextMessage, errorMessage])", "root web preview shows a real route error instead of a fake fallback");
...
assertNotIncludes(webHumanPreview, "我先看市场热度和赔率，只给你观察结果。", "root web preview does not fake prediction replies locally");
assertNotIncludes(webHumanPreview, "我来打开你的 HWallet 收款入口。", "root web preview does not fake receive replies locally");
